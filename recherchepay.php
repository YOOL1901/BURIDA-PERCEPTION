...
// FONCTION CORRIGÉE : Calculer le montant dû en partant du mois suivant jusqu'au mois en cours
...
<div class="container-fluid py-3">
    ...
    <div class="row mb-4">
        <div class="col-12">
            <div class="d-flex justify-content-between align-items-center">
                ...
                <div>
                    <a href="adminmenupay.php?id=<?php echo $_SESSION['id']; ?>" class="btn btn-black btn-sm">
                ...
                <div class="text-center flex-grow-1">
                    <a href="ajouteretsadmin.php?id=<?php echo $_SESSION['id']; ?>">
                        <img src="burida.jfif" alt="LOGO1" height="80" width="160" class="img-fluid" />
                    </a>
                </div>
                
                <!-- Bouton droit : AJOUTER ETS -->
                <div>
                    <a href="ajouteretsadmin.php?id=<?php echo $_SESSION['id']; ?>">
                    <button class="btn btn-add-ets btn-sm" onclick="addEstablishment()">
                        <i class="bi bi-plus-circle me-1"></i> AJOUTER ETS
                    </button>
                    </a>
                </div>
            </div>
    ...
    try {
        // Récupération de tous les établissements POUR LE BUREAU DE LA SESSION
            ...
            <div class="table-responsive">
                <table class="table table-striped table-hover mb-0" id="facturesTable">
                    ...
                    <tbody>
                        <?php 
                        try {
                            // CORRECTION : Requête préparée avec paramètre
                                ...
                                echo '<tr data-id="'.$id_ets_safe.'">
                                    ...
                                    <td>
                                        <div class="btn-group btn-group-sm" role="group">
                                            <a href="etatindivadmin1.php?affichclient='.$id_ets_safe.'&nom='.urlencode($user['denomination']).'&ville='.urlencode($user['localite']).'&client='.urlencode($user['nom']).'&mensuel='.urlencode($user['mensualite']).'" 
                                               class="btn btn-orange" 
                                               title="Voir l\'état">
                                                <i class="bi bi-eye-fill me-1"></i> Historique
                                            </a>
                                            <a href="paynormal.php?affichclient='.$id_ets_safe.'&nom='.urlencode($user['denomination']).'&ville='.urlencode($user['localite']).'&client='.urlencode($user['nom']).'&mensuel='.urlencode($user['mensualite']).'" 
        ...
        <div class="card-footer bg-white py-3">
            <div class="row">
                ...
                <div class="col-md-6 text-end">
                    <a href="adminmenupay.php?id=<?php echo $_SESSION['id']; ?>" 
...
<script>
    // Fonction pour le bouton AJOUTER ETS
    function addEstablishment() {
        alert('Fonction AJOUTER ETS - À implémenter');
        // window.location.href = 'ajouter_ets.php';
    }
        ...
        for (let i = 0; i < rows.length; i++) {
            rows[i].addEventListener('dblclick', function() {
                ...
                if (confirm('Voulez-vous voir les détails de l\'établissement : ' + nomEts + ' ?')) {
                    ...
                    window.location.href = 'etatindivadmin1.php?affichclient=' + idEts + 
                                          '&nom=' + denomination + 
                                          '&ville=' + ville + 
                                          '&client=' + client + 
                                          '&mensuel=' + mensuel;
